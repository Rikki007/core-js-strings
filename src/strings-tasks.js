...
function removeFirstOccurrences(str, value) {
  const startEntrance = str.indexOf(value);
  const endEntrance = str.indexOf(value) + value.length;
  return str.indexOf(value) === -1
    ? str
    : str.substring(0, startEntrance).concat(str.substring(endEntrance));
}
...
function removeLastOccurrences(str, value) {
  const startEntrance = str.lastIndexOf(value);
  const endEntrance = str.lastIndexOf(value) + value.length;
  return str.indexOf(value) === -1
    ? str
    : str.substring(0, startEntrance).concat(str.substring(endEntrance));
}
...
function sumOfCodes(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    sum += str[i].toString().charCodeAt(0);
  }
  return sum;
}
...
function isPalindrome(str) {
  ...
  const str1 = str
    ...
    .filter((item) => !mapa.includes(item))
    .join('');
  const rev = str
    .split('')
    .reverse()
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === str[i].toLowerCase()) {
      finStr += str[i].toUpperCase();
    } else {
      finStr += str[i].toLowerCase();
    }
  }
  return finStr;
}
...
function encodeToRot13(str) {
  const inputMap = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const outputMap = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let cipher = '';
  for (let i = 0; i < str.length; i += 1) {
    if (inputMap.indexOf(str[i]) === -1) {
      cipher += str[i];
    } else {
      cipher += outputMap[inputMap.indexOf(str[i])];
    }
  }
  return cipher;
}
